...
- 이미지는 제외한다.
- 본문은 기존 공개 저널 글과 유사하게 `<h2>`, `<h3>`, `<p>`, `<strong>`, `<br>` 중심의 semantic HTML로 저장한다.
...
신규 글은 이미 올라간 `journal.fightingspirit.kr/journal/` 글들과 같은 결을 유지해야 한다. 저널업로더는 `--strict-seo`에서 아래 경고도 실패로 처리한다.

- 본문 HTML 태그는 `<h2>`, `<h3>`, `<p>`, `<strong>`, `<br>`만 사용한다.
- 본문 내부에 `class`, `style`, `<div>`, `<span>`, `<ul>`, `<ol>`, `<table>`을 넣지 않는다.
- 본문은 최소 8개 문단, 1,800자 이상을 기준으로 삼는다. 초안이 짧으면 올리기 전에 보강한다.
- `FAQ`라고 들어온 초안도 공개 글과 맞게 `<h2>자주 묻는 질문</h2>`으로 정규화한다.
- 문체는 선비적인 법률 해설문이어야 한다. 구어체, 광고성 CTA, 과장 표현, 승소 보장 표현을 넣지 않는다.
- 작성자 라인 `<p>작성: 조국환 변호사팀 | AUCTORITAS LAB</p>`은 본문 마지막에 한 번만 둔다.
- 기존 글처럼 쟁점 설명, 판단 기준, 증거 자료, 원상회복/절차/FAQ 등 실무 검토 순서가 드러나야 한다.
...
결과는 `work/prepared-posts.json`에 저장된다. 태그가 8개 미만이거나 제목·본문이 비면 실패한다. 검색 구조와 기존 글 유사성 문제는 `warnings`로 표시된다. 경고까지 실패로 보고 싶으면 `--strict-seo`를 붙인다.
